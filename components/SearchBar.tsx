  ...
  return (
    <form onSubmit={handleSubmit} className={isHero ? "w-full max-w-3xl mx-auto" : "w-full"}>
      {/* A2 — the row must WRAP. A <select> sizes to its widest <option>, and its
          ...
          non-shrinkable selects plus the Search button the hero row's content
          overflowed the 1440px viewport, clipping the button and putting the
          document into horizontal scroll. sm:flex-wrap lets a control drop to a
          second line instead of being cut off, and min-w-0 lets the text input
